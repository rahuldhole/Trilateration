...
//--------------------------------------------------///
//--------------------------------------------------///
//Comment this block before 						///
////This are the demo radius values					///
$Ar = 1080.9;										///
$Br = 360.3;										///
$Cr = 998.19;										///
//--------------------------------------------------///
...
//Trilateration after resolving 3D error
echo "<h2>Resolution of 3D Error in Trilateration</h2>";
//Altitude of reference nodes is known
$Ah = 48.854; // altitude (meters)
$Bh = 30.541;
$Ch = 52.542;
//Altitude received from target node
$Lh = 0.254;
//A*-Lh=OppSide and Hypotenuse is radius of circle
$Ah = 40;
$Lh = 10;
...
$angleWithA = asin($AltDiffAL/$Ar);
$angleWithB = asin($AltDiffBL/$Br);
$angleWithC = asin($AltDiffCL/$Cr);

//3D Error Corrected Radius
$Ar = $Ar*cos($angleWithA);
$Br = $Br*cos($angleWithB);
$Cr = $Cr*cos($angleWithC);

list($Lx, $Ly) = Trilateration($Ax, $Ay, $Bx, $By, $Cx, $Cy, $Ar, $Br, $Cr, $Alat, $Clat, $Along, $Clong, $R);
/////////////

echo "<hr /><h2>Comparing radius with new (Here I feel like saviour)</h2><br />";
echo "<b>Old Radius - New Radius</b><br />";
echo $ArOld." - ".$Ar." = ".abs($ArOld-$Ar)."<br />";
echo $BrOld." - ".$Br." = ".abs($BrOld-$Br)."<br />";
echo $CrOld." - ".$Cr." = ".abs($CrOld-$Cr)."<br />";
...
function Trilateration($Ax, $Ay, $Bx, $By, $Cx, $Cy, $Ar, $Br, $Cr, $Alat, $Clat, $Along, $Clong, $R){
	...
	//Method 2
	/**************Original Formula of Method 2
	Ref.: https://www.movable-type.co.uk/scripts/latlong.html
	lat2: =ASIN(SIN(lat1)*COS(d/R) + COS(lat1)*SIN(d/R)*COS(brng))
	lon2: =lon1 + ATAN2(COS(d/R)-SIN(lat1)*SIN(lat2), SIN(brng)*SIN(d/R)*COS(lat1))
	******/
	$tmp = $ALdist/$R;
	$lat2 = asin(sin($Alat)*cos($tmp)+cos($Alat)*sin($tmp)*cos($ALtoX_Angle));
	$long2 = $Along + atan2(cos($tmp)-sin($Alat)*sin($lat2), sin($ALtoX_Angle)*sin($tmp)*cos($Alat));
